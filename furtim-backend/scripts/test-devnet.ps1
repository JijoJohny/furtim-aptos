# Test the deployed Furtim contracts on Devnet
# PowerShell script for Windows

Write-Host "🧪 Testing Furtim stealth address contracts on Devnet..." -ForegroundColor Green
...
aptos move run --profile devnet --function-id "${deployedAddress}::stealth_address::register_stealth_address" --args "hex:$SCAN_PUBLIC_KEY" "hex:$SPEND_PUBLIC_KEY"
...
aptos move run --profile devnet --function-id "${deployedAddress}::stealth_address::create_stealth_payment" --args "address:$STEALTH_ADDRESS" "hex:$EPHEMERAL_PUBLIC_KEY" "u64:$AMOUNT" "string:$COIN_TYPE" "hex:$TX_HASH"
...
aptos move run --profile devnet --function-id "${deployedAddress}::stealth_address::claim_stealth_payment" --args "u64:$PAYMENT_ID" "address:$STEALTH_ADDRESS" "hex:$EPHEMERAL_PUBLIC_KEY" "hex:$SHARED_SECRET_PROOF"
...
aptos move run --profile devnet --function-id "${deployedAddress}::stealth_address::deactivate_stealth_address"
...
aptos move run --profile devnet --function-id "${deployedAddress}::stealth_address::reactivate_stealth_address" --args "hex:$SCAN_PUBLIC_KEY" "hex:$SPEND_PUBLIC_KEY"

Write-Host "✅ Stealth address reactivated successfully!" -ForegroundColor Green

Write-Host ""
Write-Host "🎉 All stealth address contract tests passed on Devnet!" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "🚀 Your stealth address system is ready on Devnet!" -ForegroundColor Green

...
# Updated PowerShell script for Windows (Aptos CLI 7.x)
...
# Get the deployed address of the local account
$localAccount = (aptos account list --profile local | ConvertFrom-Json).Result[0].address

# Register a test stealth address
aptos move run --function-id "${localAccount}::stealth_address::register_stealth_address" --args "hex:$SCAN_PUBLIC_KEY" "hex:$SPEND_PUBLIC_KEY"
...
aptos move run --function-id "${localAccount}::stealth_address::create_stealth_payment" --args "address:$STEALTH_ADDRESS" "hex:$EPHEMERAL_PUBLIC_KEY" "u64:$AMOUNT" "string:$COIN_TYPE" "hex:$TX_HASH"
...
aptos move run --function-id "${localAccount}::stealth_address::claim_stealth_payment" --args "u64:$PAYMENT_ID" "address:$STEALTH_ADDRESS" "hex:$EPHEMERAL_PUBLIC_KEY" "hex:$SHARED_SECRET_PROOF"
...
aptos move view --function-id "${localAccount}::stealth_address::get_stealth_address_info" --args "address:$STEALTH_ADDRESS"
...
aptos move run --function-id "${localAccount}::stealth_address::deactivate_stealth_address"
...
aptos move run --function-id "${localAccount}::stealth_address::reactivate_stealth_address"

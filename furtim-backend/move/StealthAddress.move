module furtim::stealth_address {
    ...
    // The scan_public_key is used to derive one-time stealth addresses from payment links
    public entry fun register_stealth_address(
        account: &signer,
        scan_public_key: vector<u8>, // Used to derive one-time addresses from payment links
        spend_public_key: vector<u8>,
    ...
    // The sender generates ephemeral keypair and shares ephemeral public key on-chain
    public entry fun create_stealth_payment(
        account: &signer,
        stealth_address: address,
        ephemeral_public_key: vector<u8>, // Ephemeral public key generated by sender
        amount: u64,
        coin_type: String,
        tx_hash: vector<u8>,
    ) acquires StealthAddressRegistry {
